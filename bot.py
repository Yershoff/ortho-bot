...
FAQ: dict[str, tuple[str, str]] = {
    "pain": (
        ...
        "Это нормально! После установки брекетов или смены дуги зубы могут "
        "ныть 3–5 дней — они начали двигаться, так и задумано.\n\n"
        "Что поможет:\n"
        ...
        "⚠️ Если боль острая, не проходит больше недели или что-то "
    ...
    "bracket_off": (
        ...
        "2. Если совсем отвалился — сохраните его и принесите на приём\n"
    ...
    "food": (
        "🍎 Что нельзя есть",
        "С брекетами избегаем всего, что может отклеить замки или погнуть дугу:\n\n"
        "❌ Твёрдое: орехи, сухари, леденцы, целые яблоки и морковь (режьте!)\n"
        "❌ Липкое: ириски, жвачка, нуга, карамель\n"
    ...
    "hygiene": (
        ...
        "2. Монопучковая щётка — вокруг каждого замка\n"
        "3. Ёршики — под дугой между брекетами\n"
        "4. Ирригатор — вечером, лучший друг брекетоносца\n"
        "5. Ополаскиватель с фтором\n\n"
        "Плохая гигиена = белые пятна на эмали после снятия брекетов. "
        "Не хотим этого! 🙏",
    ),
    "aligners": (
        "😬 Правила ношения элайнеров",
        "• Носим 20–22 часа в сутки — снимаем только поесть и почистить зубы\n"
        "• Меняем капы строго по графику, который назначил врач\n"
        "• В капах пьём только воду (чай/кофе окрашивают и деформируют)\n"
        "• Храним в контейнере — не в салфетке! (главная причина потерь 😅)\n"
        "• Моем капы прохладной водой с мылом, НЕ горячей\n\n"
    ...
    "duration": (
        ...
        "Что ускоряет лечение: являться на все приёмы вовремя, "
        "беречь брекеты, носить эластики/капы сколько назначено.\n\n"
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(
        ...
        "• подсказать, что купить для ухода\n"
        "• передать врачу фото, если что-то беспокоит\n"
        "• напомнить о визите",
...
async def doctor_hint(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(
        ...
        "Команды: /patients — список пациентов, "
